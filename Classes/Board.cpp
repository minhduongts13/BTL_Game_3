...
Board* Board::create(bool isboard) {
    ...
    if (board) { // Thay bằng hình mặc định
        ...
        if (isboard) {
            ...
            board->setPosition(Vec2(0, visibleSize.height - board->getContentSize().height / 1.5));  // Góc trên
            ...
            Size visibleSize = Director::getInstance()->getVisibleSize();
            Vec2 origin = Director::getInstance()->getVisibleOrigin();
            board->setPosition(Vec2(50, visibleSize.height - board->getContentSize().height/2 - 15));  // Góc trên
        }
        board->position = board->getPosition();
        board->initFrame();
        return board;
